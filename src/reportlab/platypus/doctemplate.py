...
class BaseDocTemplate:
    ...
    def __init__(self, filename, **kw):
        ...
        #Page numbers related
        if kw.has_key("canvastype") and kw['canvastype'] == "NumberedCanvas":
            if (kw.has_key("alignment") and kw.has_key("x") and kw.has_key("y") and kw.has_key("fontName") and kw.has_key("fontSize")):
                self.canvasAttr = {
                        'alignment': kw['alignment'],
                        'x' : kw['x'],
                        'y' : kw['y'],
                        'fontName' : kw['fontName'],
    ...
    def _startBuild(self, filename=None, canvasmaker=canvas.Canvas):
        ...
        if self.canvasType == "NumberedCanvas":
            self.canv = canvas.NumberedCanvas(filename or self.filename,
                                ...
                                alignment=self.canvasAttr['alignment'], 
                                x=float(self.canvasAttr['x']), y=float(self.canvasAttr['y']), 
                                font=(self.canvasAttr['fontName'], int(self.canvasAttr['fontSize'])),
                                )
        else:
            self.canv = canvas.Canvas(filename or self.filename,
                                pagesize=self.pagesize,
                                invariant=self.invariant,
                                pageCompression=self.pageCompression,
                                enforceColorSpace=self.enforceColorSpace,
